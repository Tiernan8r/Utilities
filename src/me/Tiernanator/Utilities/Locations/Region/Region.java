	...
	public boolean contains(BuildingMaterial material) {
		...
		for (Block block : allBlocks()) {
			if (block.getType().equals(material.getMaterial())) {
				if (block.getData() == (byte) material.getDamage()) {
					return true;
				}
			}
		}
...
//					return true;
	...
	/**
	 * Set all the blocks within the Region to the given BuildingMaterial Type, instantly.
	 *
	 * @param material
	 *            the BuildingMaterial type
	 */
	...
	/**
	 * Change all the solid blocks within the Region to the given BuildingMaterial Type, instantly.
	 *
	 * @param material
	 *            the BuildingMaterial type
	 * @param damage
	...
	/**
	 * Change all the solid blocks within the Region to the given BuildingMaterial.
	 *
	 * @param material
	 *            the BuildingMaterial type
	 * @param damage
	...
	/**
	 * Replace the blocks of a certain type within the Region with another type, instantly.
	 *
	 * @param original
		...
		for (Block block : this.allBlocks) {
			if (block.getType() == original.getMaterial()) {
				...
				if (block.getData() == (byte) 99) {
					...
					block.setData((byte) replacement.getDamage());
				}
				if (block.getData() == (byte) original.getDamage()) {
					block.setType(replacement.getMaterial());
					block.setData((byte) replacement.getDamage());
				}
			}
	...
	public void remove(BuildingMaterial material) {
		...
		for (Block block : this.allBlocks) {
			if (block.getType() == material.getMaterial() && block.getData() == (byte) material.getDamage()) {
				block.setType(Material.AIR);
			}
		}
	}
