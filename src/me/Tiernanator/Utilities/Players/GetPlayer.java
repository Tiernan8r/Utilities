...
public class GetPlayer {
	...
	public static Player getPlayer(String name, CommandSender sender, ChatColor warning, ChatColor highlight) {
		...
		for (Player i : onlinePlayers) {
			...
			if (playerUUID.equals(iUUID)) {
				...
				if(player == null) {
					sender.sendMessage(highlight + name + warning + " is not currently online?");
				}

				return player;
			}
		}
		sender.sendMessage(highlight + name + warning + " has never played on the server before.");
		return null;
	}

	public static OfflinePlayer getOfflinePlayer(String name,
			CommandSender sender, ChatColor warning,
			ChatColor highlight) {
		...
		if (offlinePlayers.length == 0) {
			sender.sendMessage(
					highlight + name + warning + " has never played on the server before.");
			return null;
		}
	...
	public static OfflinePlayer functionGetOfflinePlayer(String name) {
	...
	public static Player functionGetPlayer(String name) {
